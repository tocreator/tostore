...
class SchemaManager {
  ...
  final Map<String, TableSchemaRouteEntry> routeByUid = {};
  ...
  final Map<String, List<String>> _activeUidsBySpace = {};
  ...
  Future<List<String>> getActiveUidsForSpace(String spaceName) async {
  ...
  Future<void> saveSpaceTables(String spaceName) async {
    final path = _dataStore.pathManager.getSpaceTablesBinPath(spaceName);
    final uids = _activeUidsBySpace[spaceName] ?? [];
  ...
  Future<void> renameTableSchema(
    ...
    try {
      ...
      if (routeIdx < 0) {
        throw DbException([
          SchemaValidationStatus(
            type: ResultType.devTableNotFound,
            message:
                'Schema for table $oldTableName (uid: $tableUid) does not exist',
      ...
      if (!partitionMeta.tableSchemas.containsKey(tableUidStr)) {
        throw DbException([
          GeneralStatus(
            type: ResultType.engError,
            message:
                'Schema payload not found for table $oldTableName (uid: $tableUid) in partition ${route.partitionIndex}',
            target: oldTableName,
          ),
        ]);
      }
      ...
      meta.routes[routeIdx] = newRoute;
      await saveSchemaStructure();

      _registerRouteInLookups(newRoute);
  ...
  Future<List<String>> listAllTables({bool onlyUserTables = false}) async {
    await getSchemaMeta();
    final names = uidByName.keys.toList();

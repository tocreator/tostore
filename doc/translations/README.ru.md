...
- **Дополнительно и производительность**: [Расширенные операции KV](#kv-advanced) | [Векторный поиск](#vector-advanced) | [TTL на уровне таблицы](#ttl-config) | [Эффективное разбиение на страницы](#query-pagination) | [Кэш запросов](#query-cache) | [Атомарные выражения](#atomic-expressions) | [Транзакции](#transactions)
...
### <a id="kv-advanced"></a>Расширенные операции «ключ-значение» (db.kv)

Для более сложных сценариев «ключ-значение» рекомендуется использовать пространство имен `db.kv`. Оно предоставляет более полный набор методов.
Все следующие методы поддерживают необязательный параметр `isGlobal`: `true` означает глобально общие данные, `false` (по умолчанию) — для текущего пространства.

- **Типизированное чтение (Type-Safe Getters)**
  Получайте данные сразу в целевом формате без ручного преобразования типов:
  ```dart
...
- **Атомарный счетчик (Atomic Increment)**
...
- **Исследование и управление пространством ключей**
  Поддерживает получение ключей по префиксу, общую статистику и массовое удаление:
  ```dart
  // Получить все ключи, начинающиеся с 'setting_'
  final keys = await db.kv.getKeys(prefix: 'setting_');

  // Получить общее количество пар ключ-значение в текущем пространстве
  final count = await db.kv.count();

  // Проверить, существует ли ключ и не истек ли его срок действия
  final exists = await db.kv.exists('config_cache');

  // Удалить несколько ключей одновременно
  await db.kv.removeKeys(['temp_1', 'temp_2']);

  // Очистить все данные KV текущего пространства
  await db.kv.clear();
  ```

- **Управление жизненным циклом (TTL)**
  Получение или обновление времени истечения срока действия существующих ключей:
  ```dart
  // Получить оставшееся время до истечения срока действия
  Duration? ttl = await db.kv.getTtl('token');

  // Обновить время истечения срока действия существующего ключа (истекает через 7 дней)

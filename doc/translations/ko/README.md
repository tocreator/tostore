...
ToStore는 AGI 시대와 엣지 인텔리전스 시나리오를 위해 설계된 현대적인 데이터 엔진입니다. 분산 시스템, 멀티모달 융합, 관계형 구조화 데이터, 고차원 벡터 및 비구조화 데이터 스토리지를 네이티브로 지원합니다. 셀프 라우팅(Self-Routing) 노드 아키텍처와 신경망 모방 엔진을 기반으로 구축되어 노드에 높은 자율성과 유연한 수평적 확장성을 제공하며, 성능과 데이터 규모를 논리적으로 분리합니다. ACID 트랜잭션, 복잡한 관계형 쿼리(JOIN 및 캐스케이드 외래 키), 테이블 수준 TTL, 집계 등 탄탄한 기본 성능과 함께 자동화된 운영 상태 분석 및 완전 비동기 데이터 처리를 지원하여 아키텍처 진화(스키마 변경 업그레이드, 키 변경, 대규모 데이터 작업)의 무중단·무감지화를 실현했습니다. 내장된 분산 기본 키, 원자성 표현식, 암호화 보호, 다중 공간 격리, 자원 인지형 지능적 부하 분산 및 재해/크래시 자체 복구 등의 기업급 기능을 통해 ToStore는 에이전트(Agent), 자동화 운영 등 고연속성 시나리오에서 지속적으로 진화하며 서비스를 중단 없이 제공합니다.
...
- 🔑 **스키마, 인덱스 및 효율적인 검색**
  - 순차 증가, 타임스탬프, 날짜 접두사, 단축 코드 등 다원 기본 키
  - 복합 인덱스, 벡터 인덱스, 외래 키 제약 조건 등 다차원 제약
  - 복합 조건, JOIN 연관 및 근접 벡터 융합 검색 지원

- 🤖 **지능형 운영 및 무감지 진화**
  - 상태 규격을 내장하여 자동화 운영 및 에이전트의 정확한 인식 지원
  - 아키텍처 진화, 키 변경 및 대규모 데이터 작업을 매끄럽고 무감지하게
  - 중단 없는 핫 업그레이드를 지원하여 시스템의 장기적인 안정 운용 보장

...
- [KV 고급 작업](#kv-advanced) | [일괄 작업](#bulk-operations) | [벡터 및 하이브리드 검색](#vector-advanced) | [테이블 수준 TTL](#ttl-config) | [효율적인 페이지 매김](#query-pagination) | [메모리 프로브 및 동기 검색 (peek)](#query-peek) | [쿼리 캐시](#query-cache) | [원자 표현](#atomic-expressions) | [거래](#transactions)
- [관리](#database-maintenance) | [보안설정](#security-config) | [오류 처리](#error-handling) | [성능 및 진단](#performance) | [기여](#contribute) | [AI 코딩 어시스턴트](#for-ai-coding-assistants)

## <a id="why-tostore"></a>왜 ToStore를 선택해야 할까요?

ToStore는 AGI 시대와 엣지 인텔리전스 시나리오를 위해 설계된 현대적인 데이터 엔진입니다. Self-Routing 노드 아키텍처를 기반으로 노드에 높은 자율성과 유연한 수평적 확장성을 제공하며, 성능과 데이터 규모를 논리적으로 분리합니다.

런타임 모델링과 논블로킹 실행 경로를 통해 아키텍처 진화는 항상 온라인이며 비즈니스에 완전히 투명합니다—선언적 스키마 변경, 데이터 인코딩 키 순환, 대규모 데이터 리팩토링이 모두 온라인에서 원활하게 수행됩니다. Agent와 자동화 운영을 위해 이들의 자율적 진화와 지속적 반복을 뒷받침하며, 서비스는 중단되지 않습니다.

관계형 구조화 데이터, 고차원 벡터, 비구조화 데이터를 네이티브로 지원하는 통합 데이터 엔진으로, 하이브리드 검색과 다중 경로 리콜 융합 기능을 내장하며, ACID 트랜잭션, 복잡한 관계형 쿼리(JOIN, 계단식 외래 키), 테이블 수준 TTL, 집계, 분산 기본 키 알고리즘, 원자성 표현식, 암호화, 다중 공간 격리, 자가 복구 등 엔터프라이즈급 데이터베이스 기능을 제공합니다.

컴퓨팅이 엣지 인텔리전스로 계속 전환됨에 따라, 디바이스는 더 이상 단순한 "콘텐츠 디스플레이"가 아닌 로컬 생성, 환경 인식, 실시간 의사 결정, 조정된 데이터 흐름을 담당하는 지능형 노드입니다. ToStore는 엣지에 대규모 데이터셋과 복잡한 로컬 AI 생성을 지원하는 분산 기능을 제공하며, 엣지와 클라우드 간의 깊은 인텔리전트 협업을 통해 다중 모드 상호 작용, 의미론적 벡터 하이브리드 검색, 공간 모델링, 엣지 자율 협업 등의 시나리오에 신뢰할 수 있는 데이터 기반을 제공합니다.

## <a id="key-features"></a>주요 기능

- 🤖 **런타임 진화와 지능형 운영**
  - 선언적 정의, 자동 리팩토링, 버전 관리 불필요
  - 키 순환, 스키마 변경, 대규모 리팩토링—모두 온라인, 무중단
  - 자동화 운영 및 Agent 인식을 위한 상태 사양 내장
  - 서비스 중단 없는 핫 업그레이드로 장기 안정 운용

- 🧠 **Self-Routing 분산 아키텍처**
  - 물리적 주소 지정과 데이터 규모를 분리하는 Self-Routing 노드 아키텍처
  - 고도로 자율적인 노드가 협력하여 유연한 데이터 토폴로지 구축
  - 탄력적 수평 확장, 엣지와 클라우드 간의 깊은 노드 간 협업

- 🌐 **통합 크로스 플랫폼 데이터 엔진**
  - 모바일, 데스크톱, 웹, 서버 환경 전반에 걸친 통합 API
  - 관계형 구조화 데이터, 고차원 벡터, 비구조화 데이터 커버
  - 로컬 스토리지부터 엣지-클라우드 협업까지 완전한 데이터 파이프라인

- 🔍 **구조화 쿼리 및 하이브리드 검색**
  - 복잡한 조건, JOIN, 집계 및 테이블 수준 TTL
  - 동일 쿼리 체인에서 다중 채널 리콜을 조합 가능(벡터, 구조화 등)
  - 다중 경로 리콜 융합 랭킹. 점수와 채널 진단을 쿼리 결과와 함께 반환

- ⚡ **병렬 실행 및 리소스 스케줄링**
  - 고가용성을 위한 리소스 인식 지능형 로드 스케줄링
  - 다중 노드 병렬 협업 및 작업 분해
  - 고부하 시에도 UI 애니메이션을 부드럽게 유지하는 타임 슬라이싱

- 🔐 **데이터 보안 및 격리**
  - 다중 공간 격리와 선택적 전역 공유, 다중 사용자 및 멀티테넌트 시나리오에 적합
  - ChaCha20-Poly1305 및 AES-256-GCM 암호화 내장
  - 다수의 복잡한 재해 복구 시나리오를 통해 검증됨
...
AI 어시스턴트로 ToStore 클라이언트 코드를 생성할 때는 단일 파일 명세 [`llms-full.txt`](../../../llms-full.txt)를 제공하세요. IDE에서 `@llms-full.txt`, 파일 업로드/붙여넣기, 또는 [raw URL](https://raw.githubusercontent.com/tocreator/tostore/main/llms-full.txt)을 어시스턴트 문서에 등록하면 됩니다. API 시그니처·제약·안티패턴이 포함되어 공개 API에 맞게 작성하는 데 도움이 됩니다. 인덱스: [`llms.txt`](../../../llms.txt).
...
### <a id="schema-evolution"></a>스키마 진화 (Schema Evolution)

엔진은 테이블 구조 변경(테이블/필드 추가·삭제·이름 변경, 속성 조정, 인덱스 변경 등)을 자동으로 감지하고 데이터 마이그레이션을 완료합니다. 수동 버전 관리나 마이그레이션 스크립트가 필요 없습니다. 선언적 `schemas`는 `ToStore.open()` 시 진화하고, 런타임에는 `updateSchema`로도 변경할 수 있습니다——**비즈니스에 투명**하며 읽기/쓰기는 중단되지 않습니다.

#### <a id="promote-primary-key"></a>고유 필드를 기본 키로 승격

기존 **고유·비 null** 필드를 기본 키로 승격할 수 있습니다(이름 변경 가능; 데이터가 있어도 가능; 비즈니스 투명). **`setPrimaryKeyConfig`와 동시에 변경하지 마세요**.

- **모바일(선언적 `schemas`)**: `ToStore.open()` 시 자동 감지. 대상 기본 키는 반드시 `PrimaryKeyType.none`(값은 원본 고유 필드에서 가져오며, 다른 자동 생성 유형은 지원하지 않음). 이름이 같으면 이름 매칭으로 충분하고, 이름 변경 시 `fromFieldId`에 원본 필드의 `fieldId`를 지정합니다.
- **서버(런타임)**: `updateSchema(...).promoteFieldToPrimaryKey(sourceFieldName: ..., targetPrimaryKeyName: ...)` 호출. `targetPrimaryKeyName`은 선택 사항이며, 생략하면 원본 필드명을 유지합니다.
...
일반적인 스키마 변경은 비즈니스 로직에 영향을 주지 않으며 시작을 차단하지 않습니다. 모바일 앱을 자주 강제 종료하는 등 드문 예외적인 경우(예: 비정상 종료 후 간단한 데이터 유효성 검사 및 크래시 복구)에만 시작 시간이 눈에 띄게 걸릴 수 있으며, 이때 `onStartupProgress`를 사용하여 스플래시 화면 또는 진행 표시기를 표시할 수 있습니다:
...
- **원자적 카운터 (Atomic Increment)**
  고동시성 시나리오에서 숫자 값을 안전하게 증가시키거나 감소시킵니다:
  ```dart
  // 1 증가 (기본값)
...
### <a id="vector-advanced"></a>벡터 필드, 벡터 인덱스 및 하이브리드 검색

벡터 검색은 통합된 `db.query(...).matchVector(...)` 쿼리 체인을 사용합니다. 동일 체인에서 구조화 조건과 조합하거나, 다른 리콜 분기와 다중 경로 융합할 수 있습니다. 점수와 채널 진단은 `QueryResult.retrieval`로 반환되며 `data` 행과 1:1로 대응합니다. 현재 예제는 벡터 + 구조화 경로 중심이며, 어휘·그래프 등 채널도 동일한 체인형 하이브리드 검색 모델을 따라 확장됩니다.
...
- `limit`: 반환할 결과 수(일반적인 ANN의 topK에 해당)

**결과 참고 사항**(`QueryResult`):

- 비즈니스 행 데이터는 `data`, 검색 점수와 채널 정보는 `retrieval.entries`에 있으며 `data`와 **1:1**로 대응합니다
- `entry.score`: 정규화된 유사도 / 융합 점수. 보통 `0 ~ 1` 범위이며, 클수록 더 관련성이 높음을 의미합니다
- `entry.meta['distance']`: 원시 거리(벡터 채널에서 흔함). `l2` / `cosine`의 경우 일반적으로 작을수록 더 가깝습니다
- `retrieval.fusionMethod`: 단일 채널은 대개 `single`, 다중 경로 융합 리콜은 대개 `rrf`(Reciprocal Rank Fusion)입니다
...
> [!TIP]
> **페이지 크기로 limit을 명시적으로 지정하기**: 쿼리할 때 항상 `limit`을 명시적으로 지정할 것을 강력히 권장합니다. 생략하면 엔진은 한 번에 너무 많은 데이터를 조회하는 것을 방지하기 위해 기본적으로 1000개 레코드로 제한합니다.

ToStore는 듀얼 모드 페이지 매김을 지원합니다. 목록 스크롤이나 무한 로딩의 경우, 내장된 **원활한 커서 페이지 매김**을 강력히 권장합니다. 특정 페이지로 직접 이동할 때는 일반 오프셋 페이지 매김을 사용하면 됩니다.

#### 1. 기본 페이지 매김 (오프셋 모드)
데이터 양이 적거나(예: 1만 건 이하) 특정 페이지로 정확하게 이동해야 하는 시나리오에 적합합니다.

```dart
final result = await db.query('users')
    .orderByDesc('created_at')
    .offset(40) // 처음 40개 행 건너뛰기
    .limit(20); // 20개 행 가져오기
```
> [!TIP]
> `offset`이 매우 커지면 데이터베이스가 많은 레코드를 스캔하고 버려야 하므로 성능이 선형적으로 저하됩니다. 깊은 페이지 매김이나 대용량 데이터 세트의 경우 **커서 모드**를 사용하는 것이 좋습니다.

#### 2. 커서 페이지 매김 (커서 모드 - 권장)
대용량 데이터 세트 및 무한 스크롤에 이상적입니다. 현재 페이지 데이터 스트림의 시작 위치를 기록하여 페이지 이동 시 해당 위치로 직접 찾아가 읽으므로(seek), 이전 데이터를 스캔하고 버리는 비용을 방지하여 깊은 페이지 매김에서도 항상 일정한 속도를 유지합니다.

* **자동 관리**: 페이지당 표시할 레코드 수 `limit`을 설정한 후, `next()` 또는 `prev()`만 호출하면 간단하고 빠르게 최적의 페이지 매김 성능을 얻을 수 있습니다.
* **시작 위치 오프셋**: 첫 번째 쿼리 시 `.offset(N)`을 지정하여 시작 윈도우를 탐색한 후, 이후 직접 `next()`를 호출하여 다음 페이지 데이터를 가져오는 방식을 지원합니다.
...
if (page1.hasMore) {
  ...
  print('다음 페이지 데이터 수: ${page2.data.length}');
  
  // 3. 이전 페이지 가져오기
  if (page2.hasPrev) {
    final prevPage = await page2.prev();
    print('이전 페이지 데이터: ${prevPage.data}');
  }
}
```
...
final String? nextToken = page1.nextCursorToken; // 이 토큰을 직렬화하여 클라이언트에 반환

// 클라이언트가 토큰을 포함하여 다음 페이지를 요청할 때:
if (nextToken != null) {
  final page2 = await db.query('users')
      .orderByDesc('id')
      .limit(20)
      .cursor(nextToken); // 토큰을 전달하여 정확한 위치에서 읽기
}
```

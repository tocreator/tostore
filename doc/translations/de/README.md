...
- [Fortgeschrittenes KV](#kv-advanced) | [Stapeloperationen](#bulk-operations) | [Vektor- & Hybridabruf](#vector-advanced) | [TTL auf Tabellenebene](#ttl-config) | [Effiziente Paginierung](#query-pagination) | [Speicher-Sonde und synchrone Abfrage (peek)](#query-peek) | [Abfrage-Cache](#query-cache) | [Atomare Ausdrücke](#atomic-expressions) | [Transaktionen](#transactions)
- [Administration](#database-maintenance) | [Sicherheitskonfiguration](#security-config) | [Fehlerbehandlung](#error-handling) | [Leistung und Diagnose](#performance) | [Mitwirken](#contribute) | [KI-Assistenten](#for-ai-coding-assistants)
...
Runtime-Modellierung und nicht-blockierende Ausführungspfade halten die Architekturentwicklung stets online und vollständig transparent für den Geschäftsbetrieb – deklarativer Schemawechsel, Rotation der Datenkodierungsschlüssel und umfangreiches Daten-Refactoring erfolgen nahtlos online. Ausgerichtet auf Agenten und automatisiertes O&M unterstützt es deren autonome Evolution und kontinuierliche Iteration ohne Dienstunterbrechung.

Eine einheitliche Daten-Engine mit nativer Unterstützung für relationale strukturierte Daten, hochdimensionale Vektoren und unstrukturierte Daten, mit integriertem Hybridabruf und mehrwegiger Recall-Fusion sowie Datenbankfunktionen auf Enterprise-Niveau einschließlich ACID-Transaktionen, komplexen relationalen Abfragen (JOINs, kaskadierende Fremdschlüssel), TTL auf Tabellenebene, Aggregationen sowie verteilten Primärschlüsselalgorithmen, atomaren Ausdrücken, Verschlüsselung, Multi-Space-Isolierung und Selbstheilung.

Da sich die Datenverarbeitung zunehmend in Richtung Edge Intelligence verlagert, sind Geräte nicht mehr nur „Inhaltsanzeigen". Sie sind intelligente Knoten für lokale Erzeugung, Umweltbewusstsein, Echtzeitentscheidungen und koordinierten Datenfluss. ToStore bietet dem Edge verteilte Funktionen für riesige Datensätze und komplexe lokale KI-Generierung. Die tiefe intelligente Zusammenarbeit zwischen Edge- und Cloud-Knoten schafft eine zuverlässige Datengrundlage für multimodale Interaktion, semantischen Vektor-Hybridabruf, räumliche Modellierung, autonome Edge-Zusammenarbeit und ähnliche Szenarien.
...
- 🔍 **Strukturierte Abfragen und Hybridabruf**
  - Komplexe Prädikate, JOINs, Aggregationen und TTL auf Tabellenebene
  - Mehrkanaliger Recall kann in derselben Abfragekette kombiniert werden (Vektor, strukturiert und mehr)
  - Mehrwegiges Recall-Fusion-Ranking; Scores und Kanaldiagnosen werden im Abfrageergebnis zurückgegeben
...
Wenn Sie ToStore-Clientcode mit einem KI-Assistenten erzeugen, geben Sie ihm das Einzeldatei-Korpus [`llms-full.txt`](../../../llms-full.txt) — z. B. `@llms-full.txt` in der IDE, Datei hochladen/einfügen oder die [raw URL](https://raw.githubusercontent.com/tocreator/tostore/main/llms-full.txt) in die Assistenten-Dokumentation aufnehmen. Die Datei enthält API-Signaturen, Einschränkungen und Anti-Patterns, damit das Modell zur echten öffentlichen Oberfläche passt. Index: [`llms.txt`](../../../llms.txt).
...
> Für fortgeschrittene Operationen wie typsicheres Lesen (`getInt`, `getBool`), atomares Inkrementieren, Präfixsuche, **verkettete paginierte Datensatzabfragen** (`db.kv.query()`) und Key-Space-Exploration lesen Sie bitte [**Fortgeschrittene Key-Value-Operationen (db.kv)**](#kv-advanced).
...
### <a id="schema-evolution"></a>Schemaentwicklung (Schema Evolution)

Die Engine erkennt strukturelle Änderungen automatisch (Hinzufügen, Entfernen oder Umbenennen von Tabellen/Feldern, Attributanpassungen, Indexänderungen usw.) und führt die Datenmigration durch — ohne manuelle Versionsverwaltung oder Migrationsskripte. Deklarative `schemas` entwickeln sich bei `ToStore.open()`; zur Laufzeit auch per `updateSchema` — **geschäftslogiktransparent**, Lesen und Schreiben bleiben unterbrechungsfrei.

#### <a id="promote-primary-key"></a>Eindeutiges Feld zum Primärschlüssel erheben

Ein bestehendes **eindeutiges, nicht-null**-Feld kann zum Primärschlüssel erhoben werden (Umbenennung optional; mit vorhandenen Daten; geschäftslogiktransparent). **Nicht** zusammen mit `setPrimaryKeyConfig` ändern.

- **Mobil (deklarative `schemas`)**: Automatische Erkennung bei `ToStore.open()`. Der Ziel-Primärschlüssel **muss** `PrimaryKeyType.none` sein (Werte stammen aus dem Quellfeld; andere Auto-Generierungstypen werden nicht unterstützt). Bei gleichem Namen reicht der Name; zum Umbenennen `fromFieldId` auf die `fieldId` des Quellfelds setzen.
- **Server (Laufzeit)**: `updateSchema(...).promoteFieldToPrimaryKey(sourceFieldName: ..., targetPrimaryKeyName: ...)` aufrufen. `targetPrimaryKeyName` ist optional; ohne Angabe bleibt der Quellfeldname erhalten.
...
final db = await ToStore.open(
  ...
  onStartupProgress: (progress, stage) {
    ...
    print('Startfortschritt ${(progress * 100).toStringAsFixed(0)}% [$stage]');
    // Splash-Screen / Fortschrittsbalken aktualisieren
  },
);
// Datenbank ist vollständig bereit
```
...
Für komplexere Key-Value-Szenarien wird empfohlen, den Namespace `db.kv` zu verwenden. Er bietet einen vollständigen Satz von APIs mit Speicherplatzisolierung, globaler Freigabe, mehreren Datentypen sowie verketteten komplexen Abfragen und Filtern (z. B. `db.kv.query().prefix(...).orderBy...().limit(...)` für Paginierung, Sortierung, Ablauf-Filter usw.).
...
- **Verkettete Datensatzabfragen (db.kv.query)**
  Verkettete API ähnlich zu `db.query()` zum Abfragen von Key-Value-**Datensätzen** (inkl. dekodiertem `value`) mit Paginierung. Im Gegensatz zu `getKeys` (nur Schlüsselnamen) liefert dies vollständige Datensätze.


  ```dart
  // Erste Seite: nach Präfix filtern, nach Update-Zeit absteigend, 20 pro Seite
  final page = await db.kv.query()
      .prefix('setting_')
      .orderByUpdatedAtDesc() // oder orderByKeyAsc / orderByKeyDesc / orderByUpdatedAtAsc
      .limit(20);

  for (final record in page.data) {
    // record enthält: key, value, updated_at, expires_at
    print('${record['key']} = ${record['value']}');
  }

  // Empfohlen: mit next() / prev() blättern (wie bei Tabellenabfragen, am einfachsten)
  if (page.hasMore) {
    final page2 = await page.next();
    print('Nächste Seite: ${page2.data.length}');
    if (page2.hasPrev) {
      final back = await page2.prev();
      print('Vorherige Seite: ${back.data.length}');
    }
  }

  // Offset-Paginierung (schließt sich mit cursor aus; für tiefes Blättern besser next() oben)
  final byOffset = await db.kv.query()
      .orderByKeyAsc()
      .limit(20)
      .offset(20);

  // Gesamtzahl der Treffer (ohne prefix: O(1)-Metadatenstatistik)
  ...
  // Standardmäßig werden abgelaufene Datensätze ausgefiltert; für noch nicht bereinigte:
  final withExpired = await db.kv.query()
      .includeExpired()
      .limit(20);
  ```

  Häufige Kettenmethoden:

  | Methode | Beschreibung |
  | --- | --- |
  | `prefix(String)` | Nach Key-Präfix filtern |
  | `orderByKeyAsc` / `orderByKeyDesc` | Nach Key (Primärschlüssel) sortieren |
  | `orderByUpdatedAtAsc` / `orderByUpdatedAtDesc` | Nach `updated_at` sortieren |
  | `limit(n)` | Max. Einträge pro Seite (explizit angeben empfohlen) |
  | `offset(n)` | Offset-Paginierung (löscht cursor) |
  | `cursor(token)` | Nur Sonderfälle: Paginierungs-Token über Prozesse/Netzwerke |
  | `includeExpired([true])` | Abgelaufene, noch nicht bereinigte Datensätze einschließen |
  | `count()` | Treffer zählen |
  | `first()` | Ersten Datensatz zurückgeben (ändert limit des Builders nicht) |

  Abfrageergebnis `QueryResult`: Alltagspaginierung mit `hasMore` / `hasPrev` + `next()` / `prev()`; `nextCursorToken` / `prevCursorToken` nur für client-/server- oder netzwerkübergreifende Übertragung u. Ä. (wie bei Tabellenabfragen).

- **Erkundung & Verwaltung (Discovery & Management)**
  ```dart
  // Nur Schlüsselnamen auflisten (ohne value); optional prefix / limit / offset
...
### <a id="vector-advanced"></a>Vektorfelder, Vektorindizes und Hybridabruf

Vektorabruf nutzt die einheitliche Abfragekette `db.query(...).matchVector(...)`: sie lässt sich mit strukturierten Prädikaten in derselben Kette kombinieren oder mit anderen Recall-Zweigen fusionieren. Scores und Kanaldiagnosen werden in `QueryResult.retrieval` zurückgegeben und sind 1:1 auf `data`-Zeilen ausgerichtet. Die aktuellen Beispiele fokussieren Vektor + strukturiert; lexikalische, Graph- und weitere Kanäle erweitern dasselbe verkettete Hybridabruf-Modell.
...
- `limit`: Anzahl zurückzugebender Ergebnisse (entspricht topK in typischer ANN-Nutzung)

**Ergebnisnotizen** (`QueryResult`):

- Geschäftszeilen liegen in `data`; Abruf-Scores und Kanalinfos in `retrieval.entries`, **1:1** zu `data`
- `entry.score`: normalisierter Ähnlichkeits- / Fusionsscore, typisch `0 ~ 1`; größer = relevanter
- `entry.meta['distance']`: Rohdistanz (häufig im Vektorkanal); bei `l2` / `cosine` bedeutet kleiner meist näher
- `retrieval.fusionMethod`: bei einem Kanal meist `single`; Mehrweg-Fusion typisch `rrf` (Reciprocal Rank Fusion)
...
Für die Alltagspaginierung in der App bevorzugen Sie die oben genannten `next()` / `prev()`. Cursor-Token nur bei Client-Server-APIs oder wenn der Paginierungsstatus über Prozesse/Netzwerke serialisiert werden muss:
...
* **Hinweis**: `cursor` und `offset` schließen sich gegenseitig aus; das Setzen des einen löscht das andere.
...
if (!txResult.hasErrors) {
  print('Transaktion erfolgreich übermittelt');
} else {
  print('Transaktions-Rollback aus folgenden Gründen:');
...
Es gibt zwei Kanäle für Fehler- und Ausnahme-Feedback in ToStore:

> [!NOTE]
> **Einheitliche Diagnosebasis**: Unabhängig davon, ob sie über das Antwort-Ergebnismodell (`statuses` in `DbResult`/`QueryResult`) zurückgegeben oder über fatale Ausnahmen (`statuses` in `DbException`) ausgelöst werden, basieren alle Diagnosezustände einheitlich auf dem strukturierten **`ResultStatus`**-System und teilen dieselben Statuscodes, was Konsistenz garantiert.

1. Antwort-Ergebnismodell (Result-based Response)
Für alltägliche Operationen wie Einfügen, Aktualisieren, Löschen, Abfragen, Transaktionen und Tabellenstrukturänderungen zur Laufzeit. Diese Operationen **lösen keine Ausnahmen aus**, wenn Einschränkungsverletzungen, Validierungsfehler oder ungültige Argumente auftreten. Stattdessen verpackt ToStore die Ergebnisse in `DbResult` oder `QueryResult` und zeichnet alle Diagnoseinformationen in der Statusliste auf. Dies garantiert, dass gewöhnliche geschäftslogische Fehler die Datenbank nicht unterbrechen.

- **`hasErrors`: Gibt an, ob bei der aktuellen Operation Fehler aufgetreten sind. Bei Batch-Operationen oder Transaktionen ist diese Eigenschaft `true`, wenn mindestens ein Fehler vorliegt.**
- **`statuses`: Eine detaillierte Liste aller `ResultStatus`-Diagnosen der Operation. Sie unterstützt eine 1:1-Reihenfolgeübereinstimmung, was für Batch-Operationen sehr nützlich ist.**
- **`firstPrimaryKey`: Liest den physisch generierten Primärschlüssel bei einer einzelnen Einfüge-/Schreiboperation direkt aus, ohne die `statuses` manuell parsen zu müssen.**
- **`ResultType`: Enum für die Zustandskategorie, praktisch für Verzweigungen (z. B. `isBusinessError`, `isDeveloperError`).**

2. Ausnahmen auslösen (Exception-based Throwing)
Für fatale Fehler, die durch Entwicklerfehler oder Designfehler verursacht werden (z. B. Schemaüberprüfungsfehler während `ToStore.open`, Inkompatibilität der Engine-Version, schwerwiegende Datenmigrationsfehler usw.). In diesen Fällen löst ToStore eine `DbException` aus, um die Ausführung zu stoppen und den Entwickler zur Behebung aufzufordern.

> [!WARNING]
> **Entwicklungsrichtlinien**: Gewöhnliche Geschäftsfehler dürfen keine Ausnahmen auslösen. Sie müssen im Antwort-Ergebnismodell zurückgegeben werden, um den Anwendungsbetrieb nicht zu stören.

---

### Beispiele für Fehler und Ausnahmen

#### 1. Fehlerbehandlung bei einer einzelnen Schreiboperation
...
if (result.hasErrors) {
  // Ersten Fehlertyp und Beschreibung abrufen
  print('Operation failed: [\${result.firstType.codeKey}] \${result.message}');
} else {
  print('Schreiben erfolgreich, Primärschlüssel ist: \${result.firstPrimaryKey}');
}
```

#### 2. Batch-Schreiboperationen und detaillierte Diagnose
...
if (batchResult.hasErrors) {
  print('Batch-Operation teilweise fehlgeschlagen: erfolgreich \${batchResult.successCount}, fehlgeschlagen \${batchResult.failedCount}');
  
  for (final status in batchResult.statuses) {
    final int idx = status.index;
    
    if (status is ConstraintStatus) {
      print('Index [\$idx] Einschränkungsverletzung! Tabelle! Tabelle: \${status.tableName}, Felder: \${status.fields}');
    } else if (status is InvalidArgumentStatus) {
      print('Index [\$idx] Argumentfehler! Parameter! Parameter: \${status.parameterName}, übergebener Wert: \${status.passedValue}');
    } else if (status.type != ResultType.success) {
      print('Index [\$idx] Fehler aufgetreten: [\${status.codeKey}] \${status.message}');
    }
  }
}
```

#### 3. Fatale Fehler und Abfangen von Initialisierungsausnahmen (DbException)
...
try {
  ...
  print('❌ Fatale Datenbank-Ausnahme! Fehlermeldung: \n\${e.message}');
  ...
  for (final status in e.statuses) {
    if (status is SchemaValidationStatus) {
      print('Schema-Validierung fehlgeschlagen! Tabelle! Tabelle: \${status.tableName}, Feld: \${status.field}, ungültige Konfiguration: \${status.wrongValue}');
    } else {
      print('Diagnose-Info: [\${status.codeKey}] \${status.message}');
    }
  }
}
```

Die vollständige Liste der Fehlertypen, Statuscodes, JSON-Serialisierungsformate und Feldzuordnungen finden Sie in der Spezifikation: [ToStore ResultStatus-Spezifikation für automatische Diagnose und Statusauflösung](result_status_specification.md).
...
> **Schlüsselverwaltung**
>
> | Schlüssel | Rolle | Änderung | Vollständige Datenmigration? |
> | :--- | :--- | :--- | :--- |
> | **`encodingKey`** | Datenverschlüsselungsschlüssel | Neuen Wert setzen und erneut `open` | **Ja** (zeitaufwendig) |
> | **`encryptionKey`** | Sicherheitsschlüssel; schützt `encodingKey` | Zur Laufzeit `db.rotateEncryptionKey` aufrufen | **Nein** (schnell) |
>
> Sensible Schlüssel niemals fest kodieren. Zur Gerätebindung `encryptionKey` im OS-Keychain / Keystore / Secure Enclave speichern und an die Engine übergeben.
...
**`encodingKey` ändern**: neuen Wert in `EncryptionConfig` setzen und erneut `open` aufrufen. Die Engine erkennt die Änderung und migriert Daten automatisch im Hintergrund—ohne Eingriff der Anwendung.

**`encryptionKey` rotieren** (regelmäßige Sicherheits-/Compliance-Rotation): keine Datenmigration; online ausführbar.
...
if (result.hasErrors) {
  // Fehler behandeln (falscher oldKey, encodingKey-Migration läuft usw.)
  return;
}
// Erfolg: In-Memory-Konfiguration aktualisiert; beim nächsten ToStore.open aktuellen encryptionKey übergeben
```
...
Highlights der 100K-Suite und Milliarden-Edge-Validierung (3 Runden, 2026-08-29):
...
Vollständige Diagramme und alle Operationen: **[Benchmarks](../../BENCHMARKS.md)**.
...
Wenn ToStore Ihnen hilft, geben Sie uns bitte ein ⭐️ – das ist eine der besten Möglichkeiten, das Projekt zu unterstützen. Vielen Dank!

## <a id="contribute"></a>🤝 Mitwirken

ToStore ist eine sich stetig weiterentwickelnde moderne Daten-Engine, und wir begrüßen Community-Beiträge sehr.
Ob Fehler beheben, Dokumentation verbessern, Architektur optimieren oder neue Ideen vorschlagen – Sie können über PR teilnehmen:

- 🔗 **PR einreichen**: [Pull Requests](https://github.com/tocreator/tostore/pulls)
- 📖 **Dokumentation**: [Wiki](https://github.com/tocreator/tostore)
- 📢 **Problemberichterstattung**: [GitHub Issues](https://github.com/tocreator/tostore/issues)
- 💬 **Technische Diskussion**: [GitHub Discussions](https://github.com/tocreator/tostore/discussions)

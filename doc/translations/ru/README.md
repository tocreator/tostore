...
- [Продвинутый KV](#kv-advanced) | [Массовые операции](#bulk-operations) | [Векторный поиск](#vector-advanced) | [TTL на уровне таблицы](#ttl-config) | [Эффективное разбиение на страницы](#query-pagination) | [Зонд памяти и синхронный поиск (peek)](#query-peek) | [Кэш запросов](#query-cache) | [Атомарные выражения](#atomic-expressions) | [Транзакции](#transactions)
- [Администрирование](#database-maintenance) | [Конфигурация безопасности](#security-config) | [Обработка ошибок](#error-handling) | [Производительность и диагностика](#performance) | [Вклад](#contribute)
...
Единый движок данных с нативной поддержкой реляционных структурированных данных, многомерных векторов и неструктурированных данных, с возможностями базы данных корпоративного уровня, включая транзакции ACID, сложные реляционные запросы (JOIN, каскадные внешние ключи), TTL на уровне таблиц, агрегирование, а также распределенные алгоритмы первичного ключа, атомарные выражения, шифрование, многопространственную изоляцию и автоматическое восстановление.

По мере того как вычисления продолжают смещаться в сторону периферийного интеллекта, устройства больше не являются просто «дисплеями контента», а представляют собой интеллектуальные узлы, отвечающие за локальную генерацию, восприятие окружающей среды, принятие решений в реальном времени и координацию данных. ToStore предоставляет периферии распределенные возможности для больших наборов данных и сложной локальной генерации ИИ. Глубокое интеллектуальное взаимодействие между периферийными и облачными узлами обеспечивает надежную основу данных для мультимодального взаимодействия, семантических векторов, пространственного моделирования, автономного взаимодействия на периферии и подобных сценариев.

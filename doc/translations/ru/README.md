...
- [Продвинутый KV](#kv-advanced) | [Массовые операции](#bulk-operations) | [Векторный и гибридный поиск](#vector-advanced) | [TTL на уровне таблицы](#ttl-config) | [Эффективное разбиение на страницы](#query-pagination) | [Зонд памяти и синхронный поиск (peek)](#query-peek) | [Кэш запросов](#query-cache) | [Атомарные выражения](#atomic-expressions) | [Транзакции](#transactions)
- [Администрирование](#database-maintenance) | [Конфигурация безопасности](#security-config) | [Обработка ошибок](#error-handling) | [Производительность и диагностика](#performance) | [Вклад](#contribute) | [ИИ-помощники](#for-ai-coding-assistants)

## <a id="why-tostore"></a>Почему стоит выбрать ToStore?

ToStore — это современный движок данных, разработанный для эпохи AGI и сценариев периферийного интеллекта. Построенный на архитектуре самомаршрутизируемых узлов (Self-Routing), он обеспечивает высокую автономность узлов и упругую горизонтальную масштабируемость, логически разделяя производительность и масштабы данных.

Моделирование во время выполнения и неблокирующие пути исполнения поддерживают эволюцию архитектуры постоянно онлайн и полностью прозрачной для бизнес-операций — декларативные изменения схемы, ротация ключей кодирования данных и масштабное рефакторинг данных выполняются без простоев. Ориентирован на агентов и автоматизированную эксплуатацию: поддерживает их автономную эволюцию и непрерывную итерацию без перерыва в обслуживании.

Единый движок данных с нативной поддержкой реляционных структурированных данных, многомерных векторов и неструктурированных данных, со встроенным гибридным поиском и слиянием многоканального отзыва, а также возможностями базы данных корпоративного уровня, включая транзакции ACID, сложные реляционные запросы (JOIN, каскадные внешние ключи), TTL на уровне таблиц, агрегирование, а также распределенные алгоритмы первичного ключа, атомарные выражения, шифрование, многопространственную изоляцию и автоматическое восстановление.

По мере того как вычисления продолжают смещаться в сторону периферийного интеллекта, устройства больше не являются просто «дисплеями контента», а представляют собой интеллектуальные узлы, отвечающие за локальную генерацию, восприятие окружающей среды, принятие решений в реальном времени и координацию данных. ToStore предоставляет периферии распределенные возможности для больших наборов данных и сложной локальной генерации ИИ. Глубокое интеллектуальное взаимодействие между периферийными и облачными узлами обеспечивает надежную основу данных для мультимодального взаимодействия, семантического векторного гибридного поиска, пространственного моделирования, автономного взаимодействия на периферии и подобных сценариев.

## <a id="key-features"></a>Ключевые особенности

- 🤖 **Эволюция во время выполнения и интеллектуальная эксплуатация**
  - Декларативное определение, автоматическое рефакторинг, без управления версиями
  - Ротация ключей, изменения схемы, масштабное рефакторинг — всё онлайн, без простоев
  - Встроенная спецификация состояний для автоматизированной эксплуатации и распознавания агентов
  - Горячие обновления без прерывания сервиса для долгосрочной стабильности

- 🧠 **Самомаршрутизируемая распределенная архитектура**
  - Архитектура самомаршрутизируемых узлов, отделяющая физическую адресацию от масштаба данных
  - Высокоавтономные узлы совместно создают гибкую топологию данных
  - Упругая горизонтальная масштабируемость с глубокой связью между периферийными и облачными узлами

- 🌐 **Единая кроссплатформенная система данных**
  - Единый API для мобильных, настольных, веб- и серверных сред
  - Охватывает реляционные структурированные данные, многомерные векторы и неструктурированные данные
  - Полный конвейер данных от локального хранилища до сотрудничества периферия-облако

- 🔍 **Структурированные запросы и гибридный поиск**
  - Сложные предикаты, JOIN, агрегации и TTL на уровне таблиц
  - Многоканальный отзыв можно комбинировать в одной цепочке запроса (вектор, структурированный и др.)
  - Многоканальное слияние ранжирования; оценки и диагностика каналов возвращаются в результате запроса

- ⚡ **Параллельное выполнение и планирование ресурсов**
  - Интеллектуальное планирование нагрузки с учетом ресурсов для высокой доступности
  - Многоузловое параллельное сотрудничество и декомпозиция задач
  - Временное разделение, обеспечивающее плавность анимации интерфейса даже при большой нагрузке

- 🔐 **Безопасность и изоляция данных**
  - Многопространственная изоляция с опциональным глобальным обменом данными, идеально подходит для многопользовательских и мультитенантных сценариев
  - Встроенное шифрование ChaCha20-Poly1305 и AES-256-GCM
  - Подтверждено множественными сложными сценариями аварийного восстановления
...
При генерации клиентского кода ToStore с помощью ИИ-помощника передайте ему единый файл спецификации [`llms-full.txt`](../../../llms-full.txt) — например `@llms-full.txt` в IDE, загрузите/вставьте файл или добавьте его [raw URL](https://raw.githubusercontent.com/tocreator/tostore/main/llms-full.txt) в документацию помощника. Файл содержит сигнатуры API, ограничения и антипаттерны, чтобы модель опиралась на реальную публичную поверхность. Индекс: [`llms.txt`](../../../llms.txt).
...
> [!TIP]
> **Нужно больше функций для работы с ключами и значениями?**
> Для расширенных операций, таких как безопасное чтение типов (`getInt`, `getBool`), атомарное приращение, поиск по префиксу, **цепочечные постраничные запросы записей** (`db.kv.query()`), см. раздел [**Расширенные операции «ключ-значение» (db.kv)**](#kv-advanced).
...
Обычные изменения схемы прозрачны для бизнес-логики и не блокируют запуск. Только в редких исключительных случаях, специфичных для мобильных приложений, которые часто принудительно закрываются (например, краткая проверка данных и восстановление после аварийного завершения) инициализация может занять заметное время — используйте `onStartupProgress` для отображения заставки или индикатора прогресса:
...
final db = await ToStore.open(
  ...
  onStartupProgress: (progress, stage) {
    ...
    print('Прогресс запуска ${(progress * 100).toStringAsFixed(0)}% [$stage]');
    // Обновить заставку / индикатор прогресса
  },
);
...
Этапы:
- `opening` — Загрузка конфигурации, подготовка базового движка
- `recovering` — Проверки безопасности и восстановление после сбоя
- `optimizing` — Внутренняя настройка движка и оптимизация структуры
- `ready` — Инициализация завершена, готов к использованию
...
### <a id="kv-advanced"></a>Продвинутые операции «ключ-значение» (db.kv)

Для более сложных сценариев «ключ-значение» рекомендуется использовать пространство имен `db.kv`. Оно предоставляет полный набор API с изоляцией пространства, глобальным общим доступом, поддержкой множества типов данных, а также цепочечными сложными запросами/фильтрами (например, `db.kv.query().prefix(...).orderBy...().limit(...)` для пагинации, сортировки, фильтрации по сроку действия и т. д.).

- **Базовый доступ (Basic Access)**
  ```dart
  // Установить значение (поддерживает String, int, bool, double, Map, List и т. д.)
  await db.kv.set('key', 'value', ttl: Duration(hours: 1));
  
  // Получить необработанное динамическое значение
...
- **Типизированное чтение (Type-Safe Getters)**
  Получайте данные напрямую в целевом формате без ручного преобразования:
  ```dart
...
- **Массовые операции (Bulk Operations)**
  Эффективная обработка нескольких пар «ключ-значение» за одну операцию:
  ```dart
  // Массовая установка
  await db.kv.setMany({
    'theme': 'dark',
...
- **Атомарные счетчики (Atomic Increment)**
  Безопасно увеличивайте или уменьшайте числовые значения в сценариях с высокой конкурентностью:
  ```dart
  // Увеличить на 1 (по умолчанию)
  await db.kv.setIncrement('view_count');
  // Уменьшить на 5 (передав отрицательное значение)
  await db.kv.setIncrement('stock_count', amount: -5);
  ```

- **Цепочечный запрос записей (db.kv.query)**
  Цепочечный API, аналогичный `db.query()`, для запроса **записей** «ключ-значение» (включая декодированное `value`) с поддержкой пагинации. В отличие от `getKeys` (только имена ключей), возвращает полные записи.


  ```dart
  // Первая страница: фильтр по префиксу, сортировка по времени обновления по убыванию, 20 на страницу
  final page = await db.kv.query()
      .prefix('setting_')
      .orderByUpdatedAtDesc() // или orderByKeyAsc / orderByKeyDesc / orderByUpdatedAtAsc
      .limit(20);

  for (final record in page.data) {
    // record содержит: key, value, updated_at, expires_at
    print('${record['key']} = ${record['value']}');
  }

  // Рекомендуется: листать через next() / prev() (как у табличных запросов — самый простой способ)
  if (page.hasMore) {
    final page2 = await page.next();
    print('Следующая страница: ${page2.data.length}');
    if (page2.hasPrev) {
      final back = await page2.prev();
      print('Предыдущая страница: ${back.data.length}');
    }
  }

  // Пагинация через Offset (взаимоисключающе с cursor; для глубокой пагинации предпочтите next() выше)
  final byOffset = await db.kv.query()
      .orderByKeyAsc()
      .limit(20)
      .offset(20);

  // Общее число подходящих записей (без prefix — агрегация метаданных O(1))
  ...
  // Глобальное пространство KV
  final globalPage = await db.kv.query(isGlobal: true).limit(50);

  // По умолчанию просроченные записи отфильтрованы; чтобы включить ещё не очищенные просроченные:
  final withExpired = await db.kv.query()
      .includeExpired()
      .limit(20);
  ```

  Часто используемые цепочечные методы:

  | Метод | Описание |
  | --- | --- |
  | `prefix(String)` | Фильтр по префиксу key |
  | `orderByKeyAsc` / `orderByKeyDesc` | Сортировка по key (первичный ключ) |
  | `orderByUpdatedAtAsc` / `orderByUpdatedAtDesc` | Сортировка по `updated_at` |
  | `limit(n)` | Максимум записей на странице (рекомендуется всегда указывать явно) |
  | `offset(n)` | Пагинация со смещением (сбрасывает cursor) |
  | `cursor(token)` | Только особые сценарии: передача токена пагинации между процессами/по сети |
  | `includeExpired([true])` | Включать ли просроченные, но ещё не очищенные записи |
  | `count()` | Подсчёт подходящих записей |
  | `first()` | Вернуть первую запись (не влияет на limit исходного builder) |

  Результат запроса `QueryResult`: для обычной пагинации используйте `hasMore` / `hasPrev` + `next()` / `prev()`; `nextCursorToken` / `prevCursorToken` — только для кросс-энд передачи и особых сценариев (использование как у табличных запросов).

- **Исследование и управление (Discovery & Management)**
  ```dart
  // Перечислить только имена ключей (без value); опционально prefix / limit / offset
  ...
  // Подсчитать общее количество ключей в текущем пространстве
...
- **Управление жизненным циклом (TTL)**
  Проверка или обновление настроек срока действия для существующих ключей:
  ```dart
  // Получить оставшуюся длительность
  Duration? ttl = await db.kv.getTtl('token');

  // Обновить TTL для существующего ключа (истекает через 7 дней)
  await db.kv.setTtl('token', Duration(days: 7));
  ```

- **Реактивное наблюдение (Reactive Watch)**
  ```dart
  // Наблюдение за отдельным ключом
  db.kv.watch<int>('unread_count').listen((count) => print(count));

  // Наблюдение за снимком нескольких ключей
  db.kv.watchValues(['theme', 'font_size']).listen((map) => print(map));
  ```

- **Глобальный доступ (isGlobal)**
  Все вышеперечисленные методы поддерживают необязательный параметр `isGlobal`: `true` для глобального пространства (общего для всех пространств), `false` (по умолчанию) для текущего изолированного пространства.


### <a id="bulk-operations"></a>Массовые операции (Bulk Operations)

ToStore предоставляет специализированные интерфейсы пакетной обработки, оптимизированные для высокой пропускной способности данных. Эти интерфейсы используют параллельное распределение задач и квантование времени (time-slicing) для обеспечения отзывчивости пользовательского интерфейса во время интенсивных операций записи.

| Метод | Основная цель | Требования к данным | Особенности |
| :--- | :--- | :--- | :--- |
| `batchInsert` | Массовая вставка записей | Должна содержать все поля, не допускающие null | Чистая вставка, максимальная производительность |
| `batchUpsert` | Умная синхронизация (Upsert) | **Должна содержать все поля, не допускающие null** | Полная синхронизация, идентификация по первичному ключу или уникальному полю |
| `batchUpdate` | Массовое обновление записей | **Первичный ключ или уникальное поле** + Поля для обновления | Частичное обновление существующих записей |

- **Массовая вставка (batchInsert)**
  ```dart
...
- **Интеллектуальная массовая синхронизация (batchUpsert)**
  Автоматически определяет "вставку" или "обновление" на основе первичного ключа или уникальных полей. Часто используется для полной синхронизации данных.
  > [!IMPORTANT]
  > **Требования к данным**: Поскольку может быть инициирована вставка, `batchUpsert` требует, чтобы каждая запись содержала все поля, не допускающие null (`nullable: false`).

- **Высокопроизводительное массовое обновление (batchUpdate)**
  Специально для обновления существующих записей. Каждая запись должна включать первичный ключ или уникальное поле в качестве идентификатора, а также поля, которые необходимо изменить.
  > [!TIP]
  > **Частичные обновления**: `batchUpdate` изменяет только предоставленные поля и не требует наличия всех полей, не допускающих null, что делает его идеальным для инкрементальных обновлений.
  ```dart
  await db.batchUpdate('users', [
    {'username': 'john', 'age': 27}, // Идентификация по уникальному полю 'username' и обновление 'age'
    {'id': '1002', 'status': 'active'}, // Также можно использовать первичный ключ напрямую
  ]);
  ```

> [!TIP]
> Вы можете установить `allowPartialErrors: true`, чтобы ошибки в отдельных записях (например, нарушение ограничения уникальности) не приводили к отклонению всей массовой операции.


### <a id="vector-advanced"></a>Векторные поля, векторные индексы и гибридный поиск

Векторный поиск использует единую цепочку `db.query(...).matchVector(...)`: её можно сочетать со структурированными условиями в той же цепочке или сливать с другими ветвями отзыва. Оценки и диагностика каналов возвращаются в `QueryResult.retrieval` и выровнены 1:1 со строками `data`. Текущие примеры сосредоточены на путях «вектор + структурированный»; лексические, графовые и другие каналы будут расширяться по той же модели цепочечного гибридного поиска.
...
- `limit`: число возвращаемых результатов (аналог topK в типичном ANN)

**Примечания к результату** (`QueryResult`):

- Бизнес-строки в `data`; оценки и сведения о каналах в `retrieval.entries`, выровнены **1:1** с `data`
- `entry.score`: нормализованная оценка сходства / слияния, обычно `0 ~ 1`; больше = релевантнее
- `entry.meta['distance']`: сырое расстояние (часто на векторном канале); для `l2` / `cosine` меньшее обычно означает ближе
- `retrieval.fusionMethod`: для одного канала обычно `single`; многоканальное слияние типично `rrf` (Reciprocal Rank Fusion)
...
ToStore решает, обновлять или вставлять запись, на основе первичного ключа или уникального поля, включенного в `data`. `where` здесь не поддерживается; цель конфликта определяется самими данными.
...
// По уникальному полю (запись должна содержать все поля, участвующие в ограничении уникальности, плюс обязательные поля)
...
// Массовый upsert (поддерживает атомарный режим или режим частичного успеха)
// allowPartialErrors: true означает, что некоторые строки могут не выполниться, в то время как другие пройдут успешно
...
### <a id="query-pagination"></a>Запрос и эффективная пагинация

> [!TIP]
> **Всегда указывайте `limit` в качестве размера страницы**: Настоятельно рекомендуется явно задавать `limit` в каждом запросе. Если он опущен, движок по умолчанию ограничивает выборку 1000 записями, чтобы предотвратить единовременное чтение слишком большого объема данных.

ToStore поддерживает двухрежимную пагинацию. Для бесконечной прокрутки или подгрузки списков настоятельно рекомендуется использовать встроенную **бесшовную пагинацию по курсору**; для перехода на конкретную страницу достаточно использовать обычную пагинацию со смещением (offset):

#### 1. Базовая пагинация (режим Offset)
Подходит для сценариев с небольшим объемом данных (например, до 10 тысяч строк) или когда требуется точный переход на определенную страницу.

```dart
final result = await db.query('users')
    .orderByDesc('created_at')
    .offset(40) // Пропустить первые 40 строк
    .limit(20); // Взять 20 строк
```
> [!TIP]
> При очень больших значениях `offset` базе данных приходится сканировать и отбрасывать множество записей, из-за чего производительность падает линейно. Для глубокой пагинации или больших наборов данных рекомендуется использовать **режим курсора**.

#### 2. Пагинация по курсору (режим Cursor - рекомендуется)
Идеально подходит для больших наборов данных и бесконечной прокрутки. Благодаря фиксации начального положения текущего потока данных страницы, движок при переходе напрямую находит нужную позицию (seek), минуя необходимость сканировать и отбрасывать пройденные записи. Это обеспечивает стабильную и высокую скорость даже на очень глубоких страницах.

* **Автоматическое управление**: Задайте размер страницы через `limit` и просто вызывайте методы `next()` или `prev()` на полученном результате для перехода к соседним страницам — это самый простой и быстрый способ получить оптимальную производительность.
* **Смещение начальной точки**: Поддерживается сочетание с `.offset(N)` при первом запросе для определения начального окна, после чего последующие страницы запрашиваются прямым вызовом `next()`.

```dart
// 1. Выполнение первого запроса
final page1 = await db.query('users')
    .orderByDesc('id')
    .limit(20);

// 2. Получение следующей страницы
if (page1.hasMore) {
  final page2 = await page1.next(); 
  print('Количество записей на следующей странице: \${page2.data.length}');
  
  // 3. Получение предыдущей страницы
  if (page2.hasPrev) {
    final prevPage = await page2.prev();
    print('Данные предыдущей страницы: \${prevPage.data}');
  }
}
```

##### Расширенный сценарий: Пагинация без сохранения состояния на основе токенов (Token-based Cursor)
Для повседневной пагинации в приложении предпочитайте `next()` / `prev()` выше. Токены курсора используйте только для клиент-серверных API или при сериализации состояния пагинации между процессами/по сети:
* Первоначальный запрос возвращает строки `nextCursorToken` / `prevCursorToken`.
* Последующий запрос передаёт токен через `.cursor(token)` для seek.
* **Примечание**: `cursor` и `offset` взаимно исключают друг друга; установка одного очищает другой.


```dart
// Первоначальный запрос (например, на стороне сервера API)
final page1 = await db.query('users')
    .orderByDesc('id')
    .limit(20);

final String? nextToken = page1.nextCursorToken; // Этот токен сериализуется и возвращается клиенту

// Когда клиент запрашивает следующую страницу, передавая токен:
if (nextToken != null) {
  final page2 = await db.query('users')
      .orderByDesc('id')
      .limit(20)
      .cursor(nextToken); // Передача токена для точного позиционирования и чтения
}
```

| Особенность | Режим Offset | Режим Cursor |
| :--- | :--- | :--- |
| **Производительность запросов** | Падает по мере увеличения номера страницы | Стабильно высокая при любой глубине страниц |
| **Лучше всего подходит для** | Небольших наборов данных, точных переходов | **Огромных наборов данных, бесконечной прокрутки** |
| **Согласованность при изменениях** | Изменения в данных могут приводить к пропуску или дублированию строк | Исключает дублирование и пропуски из-за изменений данных |


### <a id="query-peek"></a>Зонд памяти и синхронный поиск (peek)

Для сценариев с экстремальными требованиями к пропускной способности и задержке ToStore предоставляет серию `peek` — чисто синхронный поиск в памяти, поглощающий всплески горячих чтений непосредственно в процессе: **на edge** — миллионы запросов чтения в секунду; **на серверах** с более мощным оборудованием — десятки миллионов на машину (см. [Бенчмарки](#performance)).

> [!NOTE]
> **Только кэш в памяти**: `peek` — чистый обход памяти без планирования. При промахе кэша немедленно возвращает пустое/`null`; движок не выполняет синхронный файловый I/O (избегая блокировки цикла событий при высокой конкуренции). Для полных персистентных результатов используйте `await query()` в приложении.

#### Методы peek
| Метод | Тип возврата | Описание |
| :--- | :--- | :--- |
| `peekFirst()` | `Map<String, dynamic>?` | Одна запись; `null` при промахе кэша |
| `peek()` | `QueryResult<T>` | `QueryResult` со списком `data` и метаданными пагинации (`hasMore`, курсоры и т.д.); данные только при попадании в кэш |
| `peekExists()` | `bool` | Синхронно проверяет наличие совпадающей записи в кэше |
| `peekCount()` | `int` | Синхронно считает совпадающие записи в кэше |
| `result.peekNext()` | `QueryResult<T>` | Синхронная следующая страница при кэшированном результате пагинации |
| `result.peekPrev()` | `QueryResult<T>` | Синхронная предыдущая страница при кэшированном результате пагинации |

#### Лучшая практика: сначала зонд памяти (Peek-Through)
```dart
// Одна запись: сначала зонд памяти, при промахе — стандартный асинхронный запрос
...
> [!TIP]
> **Рекомендация**: Стандартные асинхронные запросы (`await query()`) обеспечивают долгосрочную стабильность и справедливость многозадачности через планирование событий; 100k+ QPS достаточно для большинства нагрузок. Серия `peek` предназначена для экстремальных пиков горячих чтений на уровне миллионов/десятков миллионов QPS на машину.
...
>
> Никогда не хардкодьте чувствительные ключи. Для привязки к устройству сохраните `encryptionKey` в Keychain / Keystore / защищённом анклаве ОС и передайте в движок.
...
Ключевые результаты suite 100K и edge-валидации в масштабе миллиарда (3 раунда, 2026-08-29):
...
Полные графики и все операции: **[Benchmarks](../../BENCHMARKS.md)**.
...
Если ToStore вам поможет, поставьте нам ⭐️ — это один из лучших способов поддержать проект. Большое спасибо!

## <a id="contribute"></a>🤝 Вклад

ToStore — это постоянно развивающийся современный движок данных, и мы тепло приветствуем вклад сообщества.
Будь то исправление ошибок, улучшение документации, совершенствование архитектуры или предложение новых идей — вы можете участвовать через PR:

- 🔗 **Отправить PR**: [Pull Requests](https://github.com/tocreator/tostore/pulls)
...
- 💬 **Техническое обсуждение**: [GitHub Discussions](https://github.com/tocreator/tostore/discussions)

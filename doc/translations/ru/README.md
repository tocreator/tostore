...
Обычные изменения схемы прозрачны для бизнес-логики и не блокируют запуск. Только в редких исключительных случаях, специфичных для мобильных приложений, которые часто принудительно закрываются (например, предыдущая миграция была прервана и новое изменение схемы теперь требует разрешения конфликтов, или краткая проверка данных после аварийного завершения) инициализация может занять заметное время — используйте `onStartupProgress` для отображения заставки или индикатора прогресса:
...
- `optimizing` — Эволюция схемы и миграция данных

...
> Для расширенных операций, таких как безопасное чтение типов (`getInt`, `getBool`), атомарное приращение, поиск по префиксу и исследование пространства ключей, см. раздел [**Расширенные операции «ключ-значение» (db.kv)**](#kv-advanced).
...
Для более сложных сценариев «ключ-значение» рекомендуется использовать пространство имен `db.kv`. Оно предоставляет полный набор API с изоляцией пространства, глобальным общим доступом и поддержкой множества типов данных.
...
- **Исследование и управление (Discovery & Management)**
  ```dart
  // Получить все ключи, начинающиеся с 'setting_'
...
Если вы разрабатываете API-интерфейсы клиент-сервер или вам необходимо сериализовать состояние пагинации для передачи по сети или между процессами, вы можете использовать токены курсора.
* Первоначальный запрос возвращает строки токенов `nextCursorToken` и `prevCursorToken`.
* Последующий запрос передает токен через метод `.cursor(token)` для быстрого позиционирования (seek).
* **Примечание**: `cursor` и `offset` взаимно исключают друг друга. Вызов `.cursor()` автоматически сбрасывает ранее установленный `offset`, и наоборот.

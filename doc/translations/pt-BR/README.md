...
> Para operações avançadas, como leitura segura de tipos (`getInt`, `getBool`), incremento atômico, pesquisa de prefixo, **consultas encadeadas paginadas de registros** (`db.kv.query()`), consulte [**Operações avançadas de valor-chave (db.kv)**](#kv-advanced).
...
### <a id="schema-evolution"></a>Evolução do esquema (Schema Evolution)

O mecanismo detecta automaticamente mudanças estruturais (adição, remoção ou renomeação de tabelas/campos, ajustes de atributos, mudanças de índices etc.) e conclui a migração de dados—sem versionamento manual nem scripts. Os `schemas` declarativos evoluem em `ToStore.open()`; em tempo de execução também use `updateSchema`—**transparente para o negócio**, sem interromper leituras nem escritas.

#### <a id="promote-primary-key"></a>Promover um campo único a chave primária

É possível promover um campo existente **único e não nulo** a chave primária (renomeação opcional; com dados existentes; transparente para o negócio). **Não combine** com `setPrimaryKeyConfig`.

- **Mobile (`schemas` declarativos)**: Detectado automaticamente em `ToStore.open()`. A chave primária de destino **deve** ser `PrimaryKeyType.none` (valores vêm do campo único de origem; outros tipos auto-gerados não são suportados). Se o nome coincidir, basta o nome; para renomear, informe o `fieldId` de origem em `fromFieldId`.
- **Servidor (runtime)**: Chame `updateSchema(...).promoteFieldToPrimaryKey(sourceFieldName: ..., targetPrimaryKeyName: ...)`. `targetPrimaryKeyName` é opcional; omita para manter o nome do campo de origem.
...
### Acompanhar o Progresso de Inicialização

Alterações normais de esquema são transparentes para a lógica de negócio e não bloqueiam a inicialização. Apenas em casos excepcionais raros específicos de aplicativos móveis frequentemente fechados à força (por exemplo, uma breve validação de dados e recuperação de falhas após uma saída anormal) a inicialização pode levar um tempo perceptível — use `onStartupProgress` para exibir uma tela de abertura ou um indicador de progresso:
...
final db = await ToStore.open(
  ...
  onStartupProgress: (progress, stage) {
    ...
    print('Progresso de inicialização ${(progress * 100).toStringAsFixed(0)}% [$stage]');
    // Atualizar tela de abertura / barra de progresso
  },
);
// Banco de dados totalmente pronto
```

Etapas:
- `opening` — Carregando configuração, preparando o motor base
- `recovering` — Verificações de segurança e recuperação após falha
- `optimizing` — Ajuste interno do motor e otimização estrutural
- `ready` — Inicialização concluída, pronto para uso
...
Para cenários de valor-chave mais complexos, recomenda-se usar o namespace `db.kv`. Ele fornece um conjunto completo de APIs com isolamento de espaço, compartilhamento global, diversos tipos de dados, e consultas/filtros complexos encadeados (ex.: `db.kv.query().prefix(...).orderBy...().limit(...)` para paginação, ordenação, filtro de expiração etc.).

- **Acesso Básico (Basic Access)**
  ```dart
  // Definir valor (suporta String, int, bool, double, Map, List, etc.)
  await db.kv.set('key', 'value', ttl: Duration(hours: 1));
  
  // Obter valor dinâmico bruto
...
- **Getters Seguros (Type-Safe Getters)**
  Recupere dados diretamente no formato de destino sem conversão manual:
  ```dart
...
- **Operações em lote (Bulk Operations)**
  Processe eficientemente vários pares de valor-chave em uma única operação:
  ```dart
  // Definir em lote
  await db.kv.setMany({
    'theme': 'dark',
...
- **Contadores Atômicos (Atomic Increment)**
...
- **Consulta encadeada de registros (db.kv.query)**
  API encadeada semelhante a `db.query()`, para consultar **registros** chave-valor (incluindo o `value` decodificado), com suporte a paginação. Diferente de `getKeys` (apenas nomes de chave), isto retorna registros completos.


  ```dart
  // Página inicial: filtrar por prefixo, ordenar por data de atualização desc, 20 por página
  ...
  for (final record in page.data) {
    // record contém: key, value, updated_at, expires_at
    print('${record['key']} = ${record['value']}');
  }

  // Recomendado: paginar com next() / prev() (igual às consultas de tabela, mais simples)
  if (page.hasMore) {
    final page2 = await page.next();
    print('Próxima página: ${page2.data.length}');
    if (page2.hasPrev) {
  ...
  // Paginação por Offset (mutuamente exclusiva com cursor; para páginas profundas prefira next() acima)
  final byOffset = await db.kv.query()
      .orderByKeyAsc()
      .limit(20)
      .offset(20);

  // Total de registros que correspondem (sem prefix é agregação de metadados O(1))
  final total = await db.kv.query().prefix('setting_').count();

  // Obter o primeiro registro correspondente
  ...
  // Por padrão filtra registros expirados; para incluir expirados ainda não limpos:
  final withExpired = await db.kv.query()
      .includeExpired()
      .limit(20);
  ```

  Métodos encadeados comuns:

  | Método | Descrição |
  | --- | --- |
  | `prefix(String)` | Filtrar por prefixo da key |
  | `orderByKeyAsc` / `orderByKeyDesc` | Ordenar por key (chave primária) |
  | `orderByUpdatedAtAsc` / `orderByUpdatedAtDesc` | Ordenar por `updated_at` |
  | `limit(n)` | Máximo de itens nesta página (recomendado especificar sempre) |
  | `offset(n)` | Paginação por deslocamento (limpa o cursor) |
  | `cursor(token)` | Apenas cenários especiais: passar Token de paginação entre processos/rede |
  | `includeExpired([true])` | Incluir registros expirados ainda não limpos |
  | `count()` | Contar itens correspondentes |
  | `first()` | Retornar o primeiro registro (não afeta o limit do builder original) |

  Resultado da consulta `QueryResult`: na paginação diária use `hasMore` / `hasPrev` + `next()` / `prev()`; `nextCursorToken` / `prevCursorToken` apenas para transmissão entre pontas e cenários especiais (uso igual às consultas de tabela).

- **Exploração & Gerenciamento (Discovery & Management)**
  ```dart
  // Enumerar apenas nomes de chave (sem value); opcional prefix / limit / offset
  ...
  // Contar o total de chaves no espaço atual
  final count = await db.kv.count();

  // Verificar se uma chave existe e não expirou
  final exists = await db.kv.exists('config_cache');

  // Sonda de memória (síncrona, apenas em cache — ver [Sonda de memória e recuperação síncrona (peek)](#query-peek))
  final theme = db.kv.peekGet('theme') ?? await db.kv.get('theme');
  if (db.kv.peekExists('config_cache')) { /* ... */ }

  // Limpar todos os dados KV no espaço atual
  await db.kv.clear();
  ```

- **Gerenciamento de Ciclo de Vida (TTL)**
  Inspecione ou atualize as configurações de expiração para chaves existentes:
  ```dart
  // Obter a duração restante
  Duration? ttl = await db.kv.getTtl('token');

  // Atualizar o TTL para uma chave existente (expira em 7 dias)
  await db.kv.setTtl('token', Duration(days: 7));
  ```

- **Monitoramento Reativo (Reactive Watch)**
  ```dart
  // Monitorar uma única chave
  db.kv.watch<int>('unread_count').listen((count) => print(count));

  // Monitorar um snapshot de várias chaves
  db.kv.watchValues(['theme', 'font_size']).listen((map) => print(map));
  ```

- **Compartilhamento Global (isGlobal)**
  Todos os métodos acima suportam o parâmetro opcional `isGlobal`: `true` para o espaço global (compartilhado entre todos os espaços), `false` (padrão) para o espaço isolado atual.


### <a id="bulk-operations"></a>Operações em Lote (Bulk Operations)

ToStore fornece interfaces especializadas de processamento em lote, otimizadas para alto rendimento de dados. Essas interfaces utilizam distribuição de tarefas em paralelo e escalonamento time-slicing para garantir a responsividade da interface do usuário durante operações de gravação intensivas.

| Método | Propósito Principal | Requisitos de Dados | Características |
| :--- | :--- | :--- | :--- |
| `batchInsert` | Inserir registros em lote | Deve conter todos os campos não nulos | Inserção pura, desempenho máximo |
| `batchUpsert` | Sincronização Inteligente (Upsert) | **Deve conter todos os campos não nulos** | Sincronização completa, identificado por chave primária ou campo único |
| `batchUpdate` | Atualizar registros em lote | **Chave primária ou campo único** + Campos para atualizar | Atualizações parciais para registros existentes |

- **Inserção em lote (batchInsert)**
  ```dart
...
- **Sincronização inteligente em lote (batchUpsert)**
  Identifica automaticamente "Inserir" ou "Atualizar" com base na chave primária ou campos únicos. Comum para sincronização completa de dados.
  > [!IMPORTANT]
  > **Requisitos de dados**: Como uma inserção pode ser acionada, o `batchUpsert` exige que cada registro contenha todos os campos não nulos (`nullable: false`).

- **Atualização em lote de alto desempenho (batchUpdate)**
  Especificamente para atualizar registros existentes. Cada registro deve incluir uma chave primária ou campo único como identificador, junto com os campos a serem modificados.
  > [!TIP]
  > **Atualizações parciais**: O `batchUpdate` altera apenas os campos fornecidos e não exige todos os campos não nulos, o que o torna ideal para atualizações incrementais.
  ```dart
  await db.batchUpdate('users', [
    {'username': 'john', 'age': 27}, // Identificar por campo único 'username' e atualizar 'age'
    {'id': '1002', 'status': 'active'}, // Também pode usar a chave primária diretamente
  ]);
  ```

> [!TIP]
> Você pode definir `allowPartialErrors: true` para garantir que falhas em registros individuais (por exemplo, violação de restrição única) não rejeitem toda a operação em lote.
...
ToStore decide se deve atualizar ou inserir com base na chave primária ou campo único incluído em `data`. `where` não é suportado aqui; o alvo do conflito é determinado pelos próprios dados.
...
// Por campo único (o registro deve conter todos os campos participantes de uma restrição única mais os campos obrigatórios)
...
// Batch upsert (suporta modo atômico ou modo de sucesso parcial)
// allowPartialErrors: true significa que algumas linhas podem falhar enquanto outras ainda têm sucesso
...
Para a paginação cotidiana no app, prefira `next()` / `prev()` acima. Use tokens de cursor apenas em APIs cliente-servidor ou ao serializar o estado de paginação entre processos/redes:
* A consulta inicial retorna as strings `nextCursorToken` e `prevCursorToken`.
* A consulta seguinte passa o token via `.cursor(token)` para seek.
* **Nota**: `cursor` e `offset` são mutuamente exclusivos; definir um limpa o outro.
...
### <a id="query-peek"></a>Sonda de memória e recuperação síncrona (peek)

Para cenários com requisitos extremos de throughput e latência, o ToStore oferece a série `peek` de recuperação sincrônica puramente em memória, absorvendo leituras quentes em rajada diretamente no processo: **dispositivos edge** podem sustentar milhões de leituras por segundo; **servidores** com hardware mais forte podem atingir dezenas de milhões por máquina (consulte [Benchmarks](#performance)).

> [!NOTE]
> **Somente cache em memória**: `peek` é um bypass puro em memória sem agendamento. Em caso de falha de cache, retorna vazio/`null` imediatamente; o motor não realiza E/S de arquivo síncrona (evitando bloqueio do loop de eventos sob alta concorrência). Para resultados persistentes completos, use `await query()` na aplicação.

#### Métodos peek
| Método | Tipo de retorno | Descrição |
| :--- | :--- | :--- |
| `peekFirst()` | `Map<String, dynamic>?` | Registro único; retorna `null` em falha de cache |
| `peek()` | `QueryResult<T>` | `QueryResult` com lista `data` e metadados de paginação (`hasMore`, cursores, etc.); dados apenas com acerto de cache |
| `peekExists()` | `bool` | Verifica sincronicamente se existe registro correspondente no cache |
| `peekCount()` | `int` | Conta sincronicamente registros correspondentes no cache |
| `result.peekNext()` | `QueryResult<T>` | Próxima página síncrona quando o resultado paginado está em cache |
| `result.peekPrev()` | `QueryResult<T>` | Página anterior síncrona quando o resultado paginado está em cache |

#### Melhor prática: sonda de memória primeiro (Peek-Through)
```dart
// Registro único: sonda de memória primeiro, consulta assíncrona padrão se falhar
final q = db.query('users').where('id', '=', userId);
final user = q.peekFirst() ?? await q.first();

// Consulta paginada com sonda
final listQ = db.query('users').orderByDesc('id').limit(20);
var page = listQ.peek();
if (page.data.isEmpty) page = await listQ;

if (page.hasMore) {
  final next = page.peekNext(); // acerto de cache: troca de página síncrona
...
| Método | Equivalente async | Descrição |
| :--- | :--- | :--- |
| `peekGet(key)` | `get(key)` | Sonda pontual em memória; chaves expiradas → `null` |
| `peekExists(key)` | `exists(key)` | Verificação de existência em memória |
| `db.kv.query().peek()` | `await db.kv.query()` | Sonda paginada (prefixo / ordenação / limit) |
| `db.kv.query().peekFirst()` | `await db.kv.query().first()` | Sonda do primeiro registro |

```dart
// Ponto: sonda primeiro, async em fallback
...
> [!TIP]
> **Recomendação**: Consultas assíncronas padrão (`await query()`) usam agendamento de eventos para estabilidade de longo prazo e equidade multitarefa; 100k+ QPS são suficientes para a maioria das cargas. A série `peek` é projetada para picos extremos de leitura quente em milhões/dezenas de milhões de QPS por máquina.
...
if (!txResult.hasErrors) {
  ...
  print('Transação revertida pelos seguintes motivos:');
...
Existem dois canais para feedback de erros e exceções no ToStore:

> [!NOTE]
> **Base de Diagnóstico Unificada**: Sejam retornados via modelo de resultado de resposta (`statuses` em `DbResult`/`QueryResult`) ou lançados via exceções fatais (`statuses` em `DbException`), todos os estados de diagnóstico são uniformemente baseados no sistema estruturado **`ResultStatus`** e compartilham os mesmos códigos de status, garantindo consistência.

1. Modelo de Resultado de Resposta (Result-based Response)
Para operações diárias como inserção, atualização, exclusão, consulta, transações e modificações de esquema de tabela em tempo de execução. Essas operações não lançarão exceções ao encontrar violações de restrição, falhas de validação ou argumentos inválidos. Em vez disso, o ToStore envolve os resultados usando `DbResult` ou `QueryResult`, registrando todas as informações de diagnóstico na lista de status. Isso garante que erros lógicos de negócios comuns não interrompam o banco de dados.

- **`hasErrors`: Indica se há erros na operação atual. Em operações em lote ou transações, se houver pelo menos um erro, esta propriedade é `true`.**
- **`statuses`: Uma lista detalhada de todos os diagnósticos `ResultStatus` da operação. Suporta uma correspondência de ordem 1:1, muito útil para operações em lote.**
- **`firstPrimaryKey`: Lê a chave primária gerada fisicamente de forma direta durante uma única operação de inserção/escrita sem a necessidade de analisar `statuses` manualmente.**
- **`ResultType`: Enumeração para a categoria de status, conveniente para o tratamento de ramificações e verificações (por exemplo, `isBusinessError`, `isDeveloperError`).**

2. Lançamento de Exceções (Exception-based Throwing)
Para erros fatais causados por descuido do desenvolvedor ou falhas de design (por exemplo, falha na verificação do esquema durante `ToStore.open`, incompatibilidade da versão do motor, corrupção fatal na migração de dados, etc.). Nesses casos, o ToStore lança `DbException` para interromper a execução, solicitando que o desenvolvedor retifique.

> [!WARNING]
> Diretrizes de Desenvolvimento: Erros comerciais comuns não devem lançar exceções; eles devem ser retornados no modelo de resultado de resposta para evitar a interrupção do tempo de execução do aplicativo.

---

### Exemplos de erros e exceções

#### 1. Tratamento de resposta para uma única gravação
...
if (result.hasErrors) {
  // Obter o primeiro tipo de erro e descrição
  print('Operation failed: [\${result.firstType.codeKey}] \${result.message}');
} else {
  print('Gravação bem-sucedida, a chave primária é: \${result.firstPrimaryKey}');
}
```
...
if (batchResult.hasErrors) {
  print('A operação em lote falhou parcialmente: bem-sucedida \${batchResult.successCount}, falhou \${batchResult.failedCount}');
  
  for (final status in batchResult.statuses) {
    final int idx = status.index;
    
    if (status is ConstraintStatus) {
      print('Index [\$idx] Violação de restrição! Tabela! Tabela: \${status.tableName}, campos: \${status.fields}');
    } else if (status is InvalidArgumentStatus) {
      print('Index [\$idx] Erro de argumento! Parâmetro! Parameter: \${status.parameterName}, valor passado: \${status.passedValue}');
    } else if (status.type != ResultType.success) {
      print('Index [\$idx] Ocorreu um erro: [\${status.codeKey}] \${status.message}');
    }
  }
}
```

#### 3. Erro fatal e captura de exceção de inicialização (DbException)
...
try {
  ...
  print('❌ Exceção fatal de banco de dados! Mensagem de erro: \n\${e.message}');
  ...
  for (final status in e.statuses) {
    if (status is SchemaValidationStatus) {
      print('Falha na validação do esquema! Tabela! Tabela: \${status.tableName}, campo: \${status.field}, configuração inválida: \${status.wrongValue}');
    } else {
      print('Informações de diagnóstico: [\${status.codeKey}] \${status.message}');
    }
  }
}
```

Para obter a lista completa de tipos de erro, códigos de status folha, formatos de serialização JSON e mapeamentos de campos, consulte a especificação completa: [Especificação de diagnóstico automático e resolução de status do ToStore ResultStatus](result_status_specification.md).
...
>
> | Chave | Função | Como alterar | Reescrita completa dos dados? |
> | :--- | :--- | :--- | :--- |
> | **`encodingKey`** | Chave de criptografia de dados | Definir novo valor e `open` novamente | **Sim** (lento) |
> | **`encryptionKey`** | Chave de segurança; protege `encodingKey` | Chamar `db.rotateEncryptionKey` em tempo de execução | **Não** (rápido) |
>
> Nunca codifique chaves sensíveis. Para vincular ao dispositivo, armazene `encryptionKey` no Keychain / Keystore / enclave seguro do SO e passe-o ao motor.
...
**Alterar `encodingKey`**: defina o novo valor em `EncryptionConfig` e execute `open` novamente. O mecanismo detecta a alteração e migra os dados automaticamente em segundo plano, sem intervenção da aplicação.

**Rotacionar `encryptionKey`** (rotação periódica de segurança/conformidade): sem reescrita de dados; executável online.
...
if (result.hasErrors) {
  // Tratar falha (oldKey incorreto, migração de encodingKey em andamento, etc.)
  return;
}
// Sucesso: config em memória atualizada; passe o encryptionKey mais recente no próximo ToStore.open
```
...
Se o ToStore ajudar você, dê-nos um ⭐️, é uma das melhores formas de apoiar o projeto. Muito obrigado!

## <a id="contribute"></a>🤝 Contribuir

O ToStore é um mecanismo de dados moderno em evolução contínua, e acolhemos calorosamente as contribuições da comunidade.
Seja corrigindo bugs, melhorando a documentação, refinando a arquitetura ou propondo novas ideias, você pode participar via PR:

- 🔗 **Enviar PR**: [Pull Requests](https://github.com/tocreator/tostore/pulls)
- 📖 **Documentação**: [Wiki](https://github.com/tocreator/tostore)
- 📢 **Relatório de problemas**: [GitHub Issues](https://github.com/tocreator/tostore/issues)
- 💬 **Discussão técnica**: [GitHub Discussions](https://github.com/tocreator/tostore/discussions)

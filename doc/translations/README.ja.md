...
## モバイル、デスクトップ、その他の頻繁な起動シナリオのための統合
...
ToStoreは`data`に含まれる主キーか一意キーに基づいて更新するか挿入するかを決定します。 `where` はここではサポートされていません。競合ターゲットはデータ自体によって決まります。

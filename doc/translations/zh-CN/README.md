...
ToStore 是面向 AGI 时代与边缘智能场景设计的现代化数据引擎。原生支持分布式体系、多模态融合、关系型结构化数据、高维向量与非结构化数据存储。基于自寻径节点架构 (Self-Routing)与类神经网络引擎，节点拥有较高自治性与弹性水平扩展，实现了性能表现与数据规模的逻辑解耦。兼具 ACID 事务、复杂关系查询（JOIN、级联外键）、表级 TTL、聚合计算等坚实底层能力，同时支持自动化运维状态解析与全异步数据处理，实现了架构演进（包括表结构变更升级、Key 变动与大范围数据操作）的无感知化。内置分布式主键算法、原子性表达式、加密保护、多空间隔离、资源感知智能负载调度与灾难崩溃自愈恢复等企业级特性，这使得 ToStore 在 Agent、自动化运维等高连续性场景能够持续迭代演进，服务始终在线不中断。
...
- 🔑 **模式、索引与高效检索**
  - 内置顺序递增、时间戳、日期前缀、短码等多元主键
  - 支持组合索引、向量索引、级联外键等多维约束
  - 支持复杂条件、JOIN 关联与近邻向量融合检索

- 🤖 **智能运维与无感知演进**
  - 内置状态规范，支撑自动化运维与智能体精准识别
  - 架构演进、Key变更及大范围数据操作平滑无感知
  - 支持不中断服务的热升级，保障系统长效运行

...
ToStore 是面向 AGI 时代与边缘智能场景设计的现代化数据引擎。基于自寻径节点架构 (Self-Routing)，节点拥有较高自治性与弹性水平扩展，性能表现与数据规模逻辑解耦。

运行态建模与非阻塞执行路径，架构演进全程在线、业务完全无感——表结构声明式变更、密钥轮换、海量数据重构均可在线无感完成，Agent 与自动化运维场景持续迭代演进，服务不中断。

原生支持关系型结构化数据、高维向量数据与非结构化数据的统一数据引擎，兼具工业级数据库能力，如 ACID 事务、复杂关系查询（JOIN、级联外键）、表级 TTL、聚合计算，以及分布式主键算法、原子性表达式、加密保护、多空间隔离与故障自愈等。

随着计算重心持续向端侧边缘智能偏移，终端设备不再只是"内容展示器"，而是承担局部生成、环境感知、实时决策与数据协同的智能节点。ToStore 赋予边缘端支撑海量数据与复杂 AI 局部生成的分布式能力，边云节点深度智能协同，为多模态交互、语义向量、空间建模等场景提供可靠的数据基座。
...
>
> 请勿硬编码敏感密钥。若需与设备绑定，请将 `encryptionKey` 存入系统 Keychain / Keystore / 安全芯片后再传入引擎。

```dart
final db = await ToStore.open(
  config: DataStoreConfig(
    encryptionConfig: EncryptionConfig(
      // 加密算法：none, xorObfuscation, chacha20Poly1305, aes256Gcm
      encryptionType: EncryptionType.chacha20Poly1305,

      // 数据加密密钥：加密表/索引/日志等数据；变更后引擎后台重写加密数据
      encodingKey: 'Your-Encoding-Key...',

      // 安全密钥：保护 encodingKey；可用 db.rotateEncryptionKey 在线轮换
      encryptionKey: 'Your-Secure-Encryption-Key...',

      // standard：加密范围涉及关键的表数据、B 树索引、日志载荷
      // full：全量加密，对整个引擎文件进行全方位加密
...
**`encodingKey` 变更**：在 `EncryptionConfig` 中设置新值并 `open` 即可，引擎检测到变更后会在后台自动迁移重写加密数据。

**`encryptionKey` 轮换**（安全合规定期更换）：不重写数据，在线轮换。

```dart
// 若此前未显式配置 encryptionKey，可省略 oldKey
final result = await db.rotateEncryptionKey(newKey: 'new-secure-key');
// 或：await db.rotateEncryptionKey(oldKey: 'old-key', newKey: 'new-key');
if (result.hasErrors) {
  // 处理失败（如旧密钥不正确、encodingKey 迁移进行中）
  return;
}
// 成功：下次 ToStore.open 时传入当前最新 encryptionKey
```

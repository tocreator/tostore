...
- Added configurable `EncryptionScope` (`standard` and `full`) to `EncryptionConfig` to allow users to customize the database encryption boundary, **`db.rotateEncryptionKey()`** for online `encryptionKey` rotation without rewriting encrypted data..
